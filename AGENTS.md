...
11. **[x] Step 11: 会議ビューア（Meeting Viewer）の実装（タイムスタンプ文字起こし・画像タイムライン・シーク＆速度調整オーディオプレイヤー）**:
    - **統合タイムライン**: 会議経過秒数（`seconds`）順にマージされた「文字起こしチャンク（`05:30 あってるよ` 等）」と「スクショ画像」の時系列表示。左端のタイムスタンプをクリックすると、オーディオプレイヤーがその秒数に即座にシーク＆自動再生。
    - **高機能オーディオプレイヤー**: 一時停止/再生、シークバー、再生速度変更（0.75x, 1x, 1.25x, 1.5x, 1.75x, 2x）、±5秒スキップ、現在位置の文字起こしアクティブハイライト＆自動スクロール追従。
    - **マルチソース対応**: GitHub / GitLab API（`00_Inbox/Meetings`）、Go Native Engine（`/api/meetings` によるローカルフォルダ参照・Rangeストリーミング）、およびローカルZIPファイル直接展開（ドラッグ＆ドロップ対応）から瞬時にロード可能。
    - **柔軟なフォーマット連携**: `transcript.json` 構造化データ、`README.md` からの正規表現自動抽出、Whisper/Teamsテキストの手動コピペ/インポートに対応。将来のセルフホストWhisper＋VADラッパー連携の受入基盤を確立。
